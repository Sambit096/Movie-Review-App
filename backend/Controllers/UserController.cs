    ...
    public class UserController : ControllerBase {
        ...
        public async Task<IActionResult> Login([FromBody] LoginRequest request) {
            try {
                ...
                return Ok(new { message = "Login successful!", username = user.Username, userId = user.UserId, firstName = user.FirstName, lastName = user.LastName, userType = user.UserType, notiPreference = user.NotiPreference, phonenumber = user.phonenumber});
            } catch (Exception ex) {
                 return StatusCode(500, ErrorDictionary.ErrorLibrary[500]);
            }
        }
